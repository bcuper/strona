...
//Walidacja imienia
        ...
        if (loginuz.value == $("#loginu").val()) {
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    if (data == 1) {
                        $('#divloginu').removeClass("has-error").addClass("has-success");
                        $('#spanloginuad').removeClass("glyphicon-remove").addClass("glyphicon-ok");
                        input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
                    }
                },
                error: function () {}
    ...
    //Walidacja datyutw
    $('#datautw').on('blur', function () {
        var input = $(this);
        var pattern = /^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/;
        var is_data = pattern.test(input.val());
        if (is_data) {
            $('#divdatautw').removeClass("has-error").addClass("has-success");
            $('#spandatautw').removeClass("glyphicon-remove").addClass("glyphicon-ok");
            input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
        } else {
            $('#divdatautw').removeClass("has-success").addClass("has-error");
            $('#spandatautw').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            input.next('.help-block').text("Wprowadź poprawną datę o formacie RRRR-MM-DD!").removeClass("text-success").addClass("error");
        }
    });
...
//Walidacja miejscowosci
    $('#miejscowosc').on('blur', function () {
        var input = $(this);       
        var name_length = input.val().length;
        if (name_length >= 4 ) {
            $('#divmiejscowosc').removeClass("has-error").addClass("has-success");
            input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
            $('#spanmiejscowosc').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divmiejscowosc').removeClass("has-success").addClass("has-error");
            $('#spanmiejscowosc').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            input.next('.help-block').text("Nazwa miejscowości musi mieć więcej niż 4 znaki!").removeClass("text-success").addClass("error");
        }
    });
    ...
    $('#ulica').on('blur', function () {
        ...
        var name_length = input.val().length;
        if (name_length >= 4 ) {
            $('#divulica').removeClass("has-error").addClass("has-success");
            input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
            $('#spanulica').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divulica').removeClass("has-success").addClass("has-error");
            $('#spanulica').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            input.next('.help-block').text("Nazwa ulicy musi mieć więcej niż 4 znaki!").removeClass("text-success").addClass("error");
        }
    });
    
    //walidacja nr domu
       $('#nrdomu').on('blur', function () {
        var input = $(this);
        var name_length = input.val().length;
        if (name_length >= 1 ) {
            $('#nrdomu').removeClass("has-error").addClass("has-success");
            input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
            $('#spannrdomu').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divnrdomu').removeClass("has-success").addClass("has-error");
            $('#spannrdomu').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            input.next('.help-block').text("Nr. domu nie może być pusty!").removeClass("text-success").addClass("error");
        }
    });
       //walidacja nr mieszk
       $('#nrmiesz').on('blur', function () {
        var input = $(this);
        var name_length = input.val().length;
        if (name_length >= 1 ) {
            $('#nrmiesz').removeClass("has-error").addClass("has-success");
            input.next('.help-block').text("").removeClass("has-error").addClass("text-success");
            $('#spannrmiesz').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divnrmiesz').removeClass("has-success").addClass("has-error");
            $('#spannrmiesz').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            input.next('.help-block').text("Nr. mieszkania nie może być pusty!").removeClass("text-success").addClass("error");
        }
    });
    
    //walidacja kodu pocztowego
       $('#kodpocz1').on('blur', function () {        
        var input = $(this);
        var name_length = input.val().length;
        if (name_length == 2 ) {
            $('#divkodpocztowy').removeClass("has-error").addClass("has-success");
            $('#helpkod').text("").removeClass("has-error").addClass("text-success");
            $('#spankodpocz1').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divkodpocztowy').removeClass("has-success").addClass("has-error");
            $('#spankodpocz1').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            $('#helpkod').text("Kod pocztowy jest niepoprawny!").removeClass("text-success").addClass("error");
        }
    });
    $('#kodpocz2').on('blur', function () {
        ...
        if (name_length == 3 ) {
            $('#divkodpocztowy').removeClass("has-error").addClass("has-success");
            $('#helpkod').text("").removeClass("has-error").addClass("text-success");
            $('#spankodpocz2').removeClass("glyphicon-remove").addClass("glyphicon-ok");

        } else {
            $('#divkodpocztowy').removeClass("has-success").addClass("has-error");
            $('#spankodpocz2').removeClass("glyphicon-ok").addClass("glyphicon-remove");
            $('#helpkod').text("Kod pocztowy jest niepoprawny!").removeClass("text-success").addClass("error");
        }
    });
//--------------
  ...
  $('#rejbtn').on('click', function (event) {
            ...
            if (!imie.hasClass('has-success') || !nazwisko.hasClass('has-success') || !login.hasClass('has-success') || !haslo.hasClass('has-success')
                    || !haslo2.hasClass('has-success') || !email.hasClass('has-success')) {
                event.preventDefault();
                alert("Uzupełnij poprawnie wszystkie pola!");
            } else {
...
function obliczcene() {
            var ilosc = document.getElementById('wybrilosc');
            var cenaprod = document.getElementById('cenaprod');
            var dozapl = cenaprod.value * ilosc.value;

            document.getElementById('wylcena').innerHTML = dozapl;
            document.getElementById('wylcena2').value = dozapl;
