            ...
            if (empty($_POST['loginlog']) && empty($_POST['haslolog'])) {
                ...
                if (mysqli_num_rows($result) != 1) { //sprawdzanie poprawnosci danych
                    ...
                    if ($row['active'] == 1) { //sprawdzanie czy konto aktywowane
                        $row = mysqli_fetch_array(mysqli_query($link, "SELECT imie, nazwisko, mail, id, dataurodzenia, datautworzenia, active, fotozaw, admin FROM uzytkownicy WHERE login='" . $login . "' AND haslo='" . $haslo . "'"));
                        mysqli_close($link);
